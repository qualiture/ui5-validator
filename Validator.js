sap.ui.define([
	...
	Validator.prototype._validate = function (oControl) {
		...
		if ((oControl instanceof sap.ui.core.Control || oControl instanceof sap.ui.layout.form.FormContainer || oControl instanceof sap.ui.layout
			.form.FormElement || oControl instanceof sap.m.IconTabFilter
		) && oControl.getVisible()) {
			...
			for (i = 0; i < aValidateProperties.length; i += 1) {
				if (oControl.getBinding(aValidateProperties[i])
					...
					&& oControl.getBinding(aValidateProperties[i]).getType() && oControl.getEnabled && oControl.getEnabled() === true
				) {
					try {
				...
				} else if (oControl.getRequired && oControl.getRequired() === true && oControl.getEnabled && oControl.getEnabled() === true) {
					try {
						oControlBinding = oControl.getBinding(aValidateProperties[i]);
						oExternalValue = oControl.getProperty(aValidateProperties[i]);
